...
**Current state (2026-08-30, end of day).** `main` = `af99f5e`, level with origin, clean, **375/375 offline tests green**, `dist/` current. **Wave 2 is delivered and closed: TAA-50 (ORDER/ADDRESS reads), TAA-55 (`clients/shopifyAdmin.ts`) and TAA-60 (shape catalogue)**, on top of TAA-46, TAA-48 and TAA-53 earlier the same day. **TAA-68 is also done and merged** (PR #8) — `--delivery pickup:` restored on Admin API 2025-10 by resolving through `draftOrderAvailableDeliveryOptions`/`shippingLine.shippingRateHandle` instead of the removed `DraftOrderInput.deliveryMethod`, live-confirmed both stores (US #10006, PS #3330); full detail in `ts/signoffs/TAA-68.md`. **This unblocks TAA-32** (click & collect cases). Wave 3 (TAA-47/52/57) is running in parallel sessions alongside this update — their own current-state entries are theirs to add.
...
Three things need JJ rather than a session: **TAA-47** still needs its scope decision (three options commented on the ticket, recommendation is a stock-only re-scope); **TAA-60's shape list needs sign-off**, which is its last acceptance item; and **TAA-56 should settle the refund question before writing cases**, because TAA-55 proved live on both stores that a targeted `refundCreate` returns a refund id but moves no money (`totalRefundedSet` $0, US #10000 and PS #3325).
