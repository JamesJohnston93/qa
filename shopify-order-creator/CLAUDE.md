...
**Current state (2026-08-31, wave 4 delivered).** `main` = `39ab631`, level with `origin/main`, clean apart from untracked `.claude/` and `_to_delete/`, **488/488 offline tests green**, `dist/` current. **Wave 4 is delivered, merged and pushed: TAA-54** (hold lifecycle cases TC7-12, PR #12) and **TAA-59** (orders-service assertions on the undeliverable cases, PR #11), both live-confirmed on both stores. Full detail in `ts/signoffs/TAA-54.md` and `ts/signoffs/TAA-59.md`; the briefs each lane worked from are in `ts/plans/TAA-54-plan.md` and `TAA-59-plan.md`.

Two structural additions. A new opt-in **`orders` subcommand** (`cases/ordersCases.ts`, `ordersRunner.ts`, `cli-orders.ts`, one additive dispatch branch in `index.ts`) carrying the six `os_hold_*` cases at pool slots 18-23 plus a shared add-item SKU at slot 74; it runs sequentially, has no progress tracker, and is deliberately outside the default set. And a new **`orders_table_refund` stage** in `runner.ts`, immediately after `cleanup`, gated on `hasRefund && !caseDef.rejectMode` so it runs on `undeliverable`/`partial_undeliverable` only. **The default regression set is unchanged at 12** and still green on both stores. `printHelp()` now points at the `orders` subcommand, which is the one item wave 4's file carve deliberately deferred to wrap-up.

What wave 4 settled, beyond the cases themselves. **TC25 is a null result and no code was written for it**: no fulfil-shaped event exists on `staging-orders-v2` under any spelling (four fulfilled orders, both stores), and the item-status half was already asserted by `assertOrderItemsFulfilled`. **The orders-v2 ITEM# terminal status on an undeliverable item is `REFUNDED`, not `UNDELIVERABLE`** — the latter appears only inside the `REFUND_ITEM` transaction's `itemChanges.refunded[].status`. **`REFUND_SHIPPING` only fires when the whole order goes undeliverable**; a partial leaves the ORDER row `OPEN`. And **`ORDER.status` reaches `REFUNDED` on a fully-undeliverable order** — that is the finalisation route TAA-52 could not observe, and it is TAA-33's to act on.

Two open items from wave 4, both for JJ. **Fraud-hold apply settled at 65-69s across all eight samples this session**, against TAA-57's 8.7-12.7s — consistent, isolated to `fulfillmentOrderHold` (outstanding-payment timings were normal), still inside the 120s window at ~1.7x margin rather than the ~3x it was sized for. Logged, not chased, per standing practice. And **`orders --list-cases` does not exist** — `orders --help` does name all six cases, so the information is there, but the flag the ticket's checklist named is not implemented.

*(Status update, 2026-08-31: the block below is superseded by the one above. Wave 3 (TAA-52/57) referred to as "still running" did complete and merge the same day, and wave 4 has since landed on top. Kept for the record.)*

**Current state (2026-08-30, end of day).** `main` = `996a754`, level with origin, clean, **385/385 offline tests green**, `dist/` current. **Wave 2 is delivered and closed: TAA-50 (ORDER/ADDRESS reads), TAA-55 (`clients/shopifyAdmin.ts`) and TAA-60 (shape catalogue)**, on top of TAA-46, TAA-48 and TAA-53 earlier the same day. **TAA-68 is also done and merged** (PR #8) — `--delivery pickup:` restored on Admin API 2025-10 by resolving through `draftOrderAvailableDeliveryOptions`/`shippingLine.shippingRateHandle` instead of the removed `DraftOrderInput.deliveryMethod`, live-confirmed both stores (US #10006, PS #3330); full detail in `ts/signoffs/TAA-68.md`. **This unblocks TAA-32** (click & collect cases). **TAA-47 is also done and merged** (PR #10) — `scripts/prepare-skus.js`, a stock-based SKU readiness reporter generalising `select-pool-candidates.js`'s real-stock rule to an arbitrary supplied SKU list (text or already-resolved JSON), PASS/FAIL per SKU with a distinct reason for unresolved GID vs. zero stock, plus an optional paste-ready pool-addition block for `src/variants.ts`; read-only, live-confirmed on both stores, no pool slots consumed; full detail in `ts/signoffs/TAA-47.md`. Wave 3 (TAA-52/57) is still running in parallel sessions alongside this update — their own current-state entries are theirs to add.

Next up, both now unblocked: **TAA-52** (verify modules for holds, finalisation and transactions, needs TAA-50, which it has) and **TAA-57** (admin flows with positive terminal settles, needs TAA-55, which it has). Size TAA-57's settle windows from TAA-53's measured latencies, not the LLD's 60s placeholder: the edit chain settles as three independent webhook deliveries out to roughly 42s.

Two things need JJ rather than a session: **TAA-60's shape list needs sign-off**, which is its last acceptance item; and **TAA-56 should settle the refund question before writing cases**, because TAA-55 proved live on both stores that a targeted `refundCreate` returns a refund id but moves no money (`totalRefundedSet` $0, US #10000 and PS #3325).
