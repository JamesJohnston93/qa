...
6. ~~Extend offline tests~~ — done (33 tests, `npm test`).
7. First live run against staging (`node dist/index.js --store US --cases single`), fix what breaks, then the full case set, then `--repeat 3`. Tune `PollWindows` from recorded stage timings.
