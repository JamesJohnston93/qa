...
**Current state (2026-08-30, end of day).** `main` = `65fd330`, level with origin, clean, **369/369 offline tests green**, `dist/` current. **Wave 2 is delivered and closed: TAA-50 (ORDER/ADDRESS reads), TAA-55 (`clients/shopifyAdmin.ts`) and TAA-60 (shape catalogue)**, on top of TAA-46, TAA-48 and TAA-53 earlier the same day. Nothing is in flight.
...
Four things need JJ rather than a session: **TAA-47** still needs its scope decision (three options commented on the ticket, recommendation is a stock-only re-scope); **TAA-60's shape list needs sign-off**, which is its last acceptance item; **TAA-68** is newly raised for the broken `--delivery pickup:` flag (`DraftOrderInput.deliveryMethod` is gone on 2025-10) and blocks TAA-32; and **TAA-56 should settle the refund question before writing cases**, because TAA-55 proved live on both stores that a targeted `refundCreate` returns a refund id but moves no money (`totalRefundedSet` $0, US #10000 and PS #3325).

Field-name drift worth knowing before writing assertions: the ORDER row carries `paymentMethod` singular (an array of method/amount); the click-and-collect store is `clickCollectStore` on the ITEM# row but `ccStore` on the CREATE_ORDER transaction envelope; and `finalPrice` does not exist anywhere, `grandTotal` is already net of discount. Two probes still owed before their tickets design cases: a CC-tagged `fulfillmentOrderMove` (TAA-32) and a return that closes with money movement (TAA-58). SKU pool after wave 2: US slots 53-56, 58, 59, 61, 62, 64-73 burned; 38-45 reserved by the shape catalogue; free are 46-49, 57, 60, 63 and 74-79.
