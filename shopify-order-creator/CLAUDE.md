...
**TS state (`ts/`, commits 9279f00 / 435fe42 / in progress):** runnable scaffold, now gaining real logic per TAA-13. Done: CLI (`src/cli.ts` + `run-regression.sh`), cases 1–6 declared (`src/cases/baselineCases.ts`), real Shopify client with `order { id name }` capture, reports, offline inventory-assertion tests, **`src/polling.ts`** (`pollUntil`/`StageTimeout`), **real `clients/dynamo.ts`** (AWS SDK v3, `fromSSO({profile:"staging"})`, `getStock`/`setStock`/`getAllLocationsForSku`/`seedInventory`/`zeroEverywhere`/`snapshotInventory`, always throws — no soft mode). `node_modules` is now `.gitignore`d (was previously committed; the AWS SDK v3 install added ~15MB so we switched to the standard convention — `npm install` reproduces it from `package-lock.json`). **Stubs — not real yet:** `clients/newstore.ts` (hardcoded IDs), readers (financial status hardcoded "PAID", no orders-v2/shipments schema guard yet), `--repeat` ignored, refund/cleanup stages unwired. The `ts/reports/` sample came from a dry run, NOT staging.

**Next priorities (in order — everything else is blocked on 1–2):**

1. ~~Port `regression/polling.py` → TS~~ — done (`src/polling.ts`).
2. ~~Real `clients/dynamo.ts` (inventory ops)~~ — done. Still open: port the `TABLE_SCHEMAS` + `UNCONFIRMED` guard from `regression/readers/dynamo_reader.py` into `readers/dynamoReader.ts` — readers must refuse to run on guessed schemas. Confirm schemas once via `python -m regression.schema_probe --order <recent-order-number>` (check allocated-store value format: '100' vs 'BRANCH_100').
