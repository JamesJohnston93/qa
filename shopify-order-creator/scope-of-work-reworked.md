...
*(Status update, 2026-08-23: **done, live-confirmed.** TAA-38 built `locations.ts` (store → Shopify-location GID mapping, one table per store since the two shared DC facilities still carry distinct per-shop GIDs — see its sign-off for the finding) and `verify/allocation.ts` (SKU/unit-count alignment per fulfilment, one-fulfilment-per-shipment, no fulfilment for undeliverables, location-vs-allocated-store). 6 live orders across both stores (single/combination/undeliverable) verified end-to-end. Full detail: `ts/signoffs/TAA-38.md`.)*
...
*(Status update, 2026-08-23: **all six slices done, live-confirmed on both stores.** `fulfil_single`/`fulfil_split` are now two of the regression suite's ten default cases (TAA-39), wired through the fulfil/verify-fulfilment/allocation-reflection stages TAA-36/37/38 built. One real backend defect surfaced by the `--repeat 3` consistency check, filed as **[TAA-42](https://universalstore.atlassian.net/browse/TAA-42)** and deliberately deferred (Shopify fulfilment sync sometimes never fires after a successful fulfil call, ~27% observed) — full order regression coverage is the priority before chasing it. TAA-21 workstream itself stays open pending JJ's call on TAA-42; the harness-building work described by this section is done. Full detail: `ts/signoffs/TAA-36.md` through `TAA-39.md`.)*
...
*(Status update, 2026-08-28 (later): **done — [TAA-31](https://universalstore.atlassian.net/browse/TAA-31)**, merged to `main`: `taa-31-reject-probe` (`9be60c2`) plus the slice G addendum on `taa-31-reject-transactions` (`8b18d73`). Both reject cases (`reject_reallocate`/`reject_undeliverable`) wired into `cases/`/`runner.ts`/`cli.ts` and live-confirmed on both US and PS, `--repeat 2` (JJ's call, to save time). A real cross-case race was found and fixed along the way: running `reject_undeliverable` immediately after `reject_reallocate` once let a stray nonzero read at `WEB_DC` pull one item back `ALLOCATED` instead of `UNDELIVERABLE` — root cause unconfirmed (probably a delayed backend inventory sync), fixed with a defensive second `zeroEverywhere` right before the reject call rather than chased further. ~~Remaining, low-priority: a reusable `TRANSACTION#` assertion (the shape's already been visually confirmed three times over).~~ Delivered same day in slice G: `getTransactionsByPk`/`transactionRowsFromRows` in `readers/dynamoReader.ts` and `assertRejectTransactions` in `verify/rejects.ts`, wired as a polled `reject_transactions` stage and live-confirmed on both stores. It resolved in 0.0s on every run, so the rows are already present by the time `rejectShipment()`'s own poll clears. Full detail: `ts/signoffs/TAA-31-slice-f.md`, including the slice G addendum at the end.)*

*(Status update, 2026-08-28: **in progress — [TAA-31](https://universalstore.atlassian.net/browse/TAA-31), slices A-E done, slice F/G next**, branch `taa-31-reject-probe` (not merged to `main`). Both reject case designs now proven live: reallocate (slice D) and undeliverable (slice E, `DynamoClient.zeroExceptStore` audited targeted-zero, order #9968 US, both items resolved `UNDELIVERABLE` in 28.4s). Neither is wired into the regression suite yet, and neither has been confirmed on PS — that, plus `TRANSACTION#` assertions and `--repeat 3` stability, is slice F/G. Full detail: `ts/signoffs/TAA-31-slice-a.md` through `-slice-e.md`.)*

*(Status update, 2026-08-24: **in progress — [TAA-31](https://universalstore.atlassian.net/browse/TAA-31), slices A-D done, slice E next**, branch `taa-31-reject-probe` (not merged to `main`). The reject call turned out to be a genuine sibling path, `POST /staging/reject` — NOT the same endpoint as fulfil, contrary to the original brief (`POST /staging/fulfil` with a reject-shaped body crashes 502). Rejection is per-shipment (one item in the payload triggers rejection of every item on that shipment), and per JJ (2026-08-23) is never valid post-fulfilment. `clients/reject.ts`, `flows/rejectFlow.ts` (`reallocationResolved`/`waitForReallocation`/`rejectShipment`) built and live-confirmed. Full detail: `ts/signoffs/TAA-31-slice-a.md` through `-slice-d.md`.)*
...
- Deterministic regression baseline running headlessly in Python (Phase 0), then reproduced by the TypeScript harness covering creation, inventory, allocation reflection, fulfilment, rejection/reallocation, and cancellation/refund on staging, run from a refreshed CLI with shareable reports. **Partly met:** the baseline is headless in TS and covers creation, inventory, allocation and the undeliverable→refund path, with markdown + JSON reports per run (`regression_<STORE>_<ISO8601Z>.md`/`.json`). **Still open:** allocation reflection (Workstream 2, now [TAA-38](https://universalstore.atlassian.net/browse/TAA-38)), fulfilment ([TAA-21](https://universalstore.atlassian.net/browse/TAA-21), sliced TAA-34–39, slice A built), rejection/reallocation ([TAA-31](https://universalstore.atlassian.net/browse/TAA-31), blocked behind it), and the refreshed operator CLI ([TAA-29](https://universalstore.atlassian.net/browse/TAA-29) — the `order` subcommand covers the daily-use path only). *(Ticket mapping corrected 2026-08-21; TAA-15 closed 2026-08-06 with its remaining UX scope moved to TAA-29.)*
  ...
  *(Status update, 2026-08-23: allocation reflection and fulfilment (TAA-38, TAA-34–39) are now both done and live-confirmed on both stores — the harness covers creation, inventory, allocation, allocation reflection, undeliverable→refund, and fulfilment end-to-end, 10 default cases. **Still open:** rejection/reallocation (TAA-31, next up) and the refreshed operator CLI (TAA-29). One real backend defect found during fulfilment testing, filed as [TAA-42](https://universalstore.atlassian.net/browse/TAA-42) and deliberately deferred — see `ts/signoffs/TAA-39.md`.)*

  *(Status update, 2026-08-24: TAA-31 slices A-D done (contract proved live, `RejectClient`, the reallocation-settle predicate, and the `rejectShipment()` flow, all live-confirmed) — see `ts/signoffs/TAA-31-slice-a.md` through `-slice-d.md`. **Still open:** slice E (reject → undeliverable case) and wiring both new cases into the regression suite proper; the refreshed operator CLI (TAA-29) remains untouched.)*

  *(Status update, 2026-08-28: TAA-31 slice E done — reject → undeliverable case, live-confirmed on US (order #9968, both items resolved `UNDELIVERABLE` in 28.4s). Audited targeted-zero (`DynamoClient.zeroExceptStore`, new this slice) seeds one designated store and zeroes only whatever else the audit finds nonzero, avoiding a blanket `zeroEverywhere` sweep for a SKU this workstream reuses repeatedly — see `ts/signoffs/TAA-31-slice-e.md`. **Still open:** slice F/G — wire both new reject cases (D's reallocate, E's undeliverable) into the regression suite proper, confirm both on PS, `TRANSACTION#` assertions, `--repeat 3` stability; the refreshed operator CLI (TAA-29) remains untouched.)*

  *(Status update, 2026-08-28 (later): **TAA-31 done.** Slice F/G wired both reject cases into the regression suite (10 → 12 default pipeline cases), live-confirmed on both stores under `--repeat 2`. Found and fixed a real cross-case race (`reject_undeliverable` run right after `reject_reallocate` could pull one item back `ALLOCATED` instead of `UNDELIVERABLE`) with a defensive re-zero, root cause unchased. Rejection/reallocation coverage is now in the same "done, both stores" state as allocation reflection and fulfilment. **Still open:** the refreshed operator CLI (TAA-29). The reusable `TRANSACTION#` assertion that was open here landed the same day in slice G (`8b18d73`), scoped to the reject events. See `ts/signoffs/TAA-31-slice-f.md` and its slice G addendum.)*

  *(Status update, 2026-08-28 (same day, addendum): **TAA-31 fully closed.** The `TRANSACTION#` assertion noted above is done — `DynamoReader.getTransactionsByPk`/`assertRejectTransactions` confirm exactly one `SHIPMENT_REJECTED` and one `SHIPMENT_ITEM_REJECTED` per rejected item, polled via a new `reject_transactions` stage. Live-confirmed on both stores: the check resolved in 0.0s on every single run, both cases — the transaction rows are already present by the time the reject itself settles. No known remaining scope on this ticket. See the "Slice G addendum" section in `ts/signoffs/TAA-31-slice-f.md`.)*
