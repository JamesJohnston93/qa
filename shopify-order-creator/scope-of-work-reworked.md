...
*(Status update, 2026-08-28 (later): **done — [TAA-31](https://universalstore.atlassian.net/browse/TAA-31)**, branch `taa-31-reject-probe` (not merged to `main`). Both reject cases (`reject_reallocate`/`reject_undeliverable`) wired into `cases/`/`runner.ts`/`cli.ts` and live-confirmed on both US and PS, `--repeat 2` (JJ's call, to save time). A real cross-case race was found and fixed along the way: running `reject_undeliverable` immediately after `reject_reallocate` once let a stray nonzero read at `WEB_DC` pull one item back `ALLOCATED` instead of `UNDELIVERABLE` — root cause unconfirmed (probably a delayed backend inventory sync), fixed with a defensive second `zeroEverywhere` right before the reject call rather than chased further. Remaining, low-priority: a reusable `TRANSACTION#` assertion (the shape's already been visually confirmed three times over). Full detail: `ts/signoffs/TAA-31-slice-f.md`.)*
