...
const rejects_1 = require("./verify/rejects");
...
const finalisation_1 = require("./verify/finalisation");
...
async function runCase(config, caseDef, progress) {
    ...
    const caseStages = (0, progress_1.stageSequenceFor)(hasRefund, caseDef.fulfilment, caseDef.rejectMode);
    ...
    try {
        ...
        if (caseDef.rejectMode) {
            const grouped = (0, dynamoReader_1.groupItemsByShipment)(lastItems);
            if (grouped.size !== 1) {
                throw new index_1.VerificationError("reject.shipment_count", 1, grouped.size, `order ${oname}: expected exactly one shipment before rejecting, got ${grouped.size}`);
            }
            ...
            const itemIdsToReject = caseDef.rejectMode === "undeliverable"
            ...
            const rejectResult = await (0, rejectFlow_1.rejectShipment)({ reader: dynamoReader, rejectClient: new reject_1.RejectClient(), verbose: config.verbose }, resolvedPk, originalShipmentId, itemIdsToReject);
            stageDone("reject", (Date.now() - t0) / 1000);
            if (caseDef.rejectMode === "undeliverable") {
                (0, rejects_1.assertAllUndeliverable)(rejectResult.items, oname);
            }
            else {
        ...
        if (Object.keys(caseDef.expectedRefundSkus).length > 0) {
            ...
            if (!caseDef.rejectMode) {
                ...
                const ordersTableRefund = await pollVerify(() => dynamoReader.getOrderRows(config.store, oidTail), (rows) => {
                    const items = (0, dynamoReader_1.orderItemRowsFromRows)(rows);
                    const transactions = (0, dynamoReader_1.transactionRowsFromRows)(rows);
                    const order = (0, dynamoReader_1.orderRecordFromRows)(rows);
                    for (const refundSku of refundSkus) {
                        const item = items.find((i) => i.sku === refundSku);
                        if (!item) {
                            throw new index_1.VerificationError("orders_table.item_status", "ITEM# row present", "not found yet", `order ${oname}, sku ${refundSku}`);
                        }
                        (0, orders_1.assertOrderItemStatus)(item, "REFUNDED", oname);
                        (0, transactions_1.assertTransactionPresent)(transactions, "REFUND_ITEM", oname, (0, transactions_1.refundedSkuStatusMatcher)(refundSku, "UNDELIVERABLE"));
                    }
                    if (fullyUndeliverable) {
                        (0, transactions_1.assertTransactionPresent)(transactions, "REFUND_SHIPPING", oname);
                        (0, finalisation_1.assertOrderStatus)(order, "REFUNDED", oname);
                    }
                    else {
                        (0, transactions_1.assertTransactionAbsent)(transactions, "REFUND_SHIPPING", oname);
                        (0, finalisation_1.assertOrderStatus)(order, "OPEN", oname);
                    }
                }, poll.ordersTableRefund, dynamoInterval, "orders_table_refund", config.verbose, (elapsed) => printProgress("orders_table_refund", elapsed));
...
async function run(config = (0, config_1.defaultConfig)(), tracker, repeatIndex = 0, totalRepeats = 1) {
    ...
    const resolvedTracker = tracker ??
        (0, progress_1.createProgressTracker)((0, progress_1.buildRunPlan)(pipelineNames, (name) => Object.keys(allCases[name].expectedRefundSkus).length > 0, totalRepeats, (name) => allCases[name].fulfilment, (name) => allCases[name].rejectMode), totalRepeats, pipelineNames.length, Date.now());
