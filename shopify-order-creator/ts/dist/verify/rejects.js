...
exports.assertRejectTransactions = assertRejectTransactions;
 ...
 */
function assertRejectTransactions(transactions, rejectedItemIds, orderName) {
    const shipmentRejected = transactions.filter((t) => t.event === "SHIPMENT_REJECTED");
    if (shipmentRejected.length !== 1) {
        throw new index_1.VerificationError("reject.transactions.shipment_rejected", 1, shipmentRejected.length, `order ${orderName}: expected exactly one SHIPMENT_REJECTED transaction, got ${shipmentRejected.length}`);
    }
    const itemRejectedCounts = new Map();
    ...
    if (bad.length > 0) {
        throw new index_1.VerificationError("reject.transactions.item_rejected", rejectedItemIds.map((id) => ({ id, expectedCount: 1 })), rejectedItemIds.map((id) => ({ id, actualCount: itemRejectedCounts.get(id) ?? 0 })), `order ${orderName}: expected exactly one SHIPMENT_ITEM_REJECTED transaction per rejected item, mismatch for: ${bad.join(", ")}`);
    }
}
