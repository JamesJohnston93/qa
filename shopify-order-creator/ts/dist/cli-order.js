...
exports.expandSkuQuantities = expandSkuQuantities;
exports.printOrderHelp = printOrderHelp;
exports.parseOrderArgs = parseOrderArgs;
exports.runOrderCli = runOrderCli;
...
function expandSkuQuantities(items) {
...
async function seedInventory(dynamo, skus, mode) {
    if (mode === "none") {
        return;
    }
    if (mode === "zero") {
    ...
    if (mode === "split") {
        for (const sku of skus) {
            for (const location of config_2.ALL_LOCATIONS) {
    ...
    for (const sku of skus) {
        ...
        if (current === null || current < MIN_QUANTITY_THRESHOLD) {
            await dynamo.setStock(sku, TOP_UP_QUANTITY, config_2.WEB_DC);
        }
    }
}
async function runShopifyOrder(config) {
    ...
    if (unknown.length > 0) {
        throw new Error(`SKUs not in ${config.store} variant map: ${JSON.stringify(unknown)}. Known: ${Object.keys(variants).join(", ")}`);
    }
    if (config.seed !== "none") {
        const dynamo = new dynamo_1.DynamoClient(regressionConfig);
        await seedInventory(dynamo, skus, config.seed);
        console.log(`Inventory seeded (${config.seed}) for ${skus.join(", ")}`);
    }
    const baseCustomer = (0, config_1.customerFor)(regressionConfig);
...
async function runNewStoreOrder(config) {
    const skuList = expandSkuQuantities(config.items);
    const shopify = new shopify_1.ShopifyClient(config.store);
    const newstore = new newstore_1.NewStoreClient();
    const inject = config.ns === "sfs" ? newstoreOrders_1.injectSfsOrder : newstoreOrders_1.injectOtcOrder;
    ...
    try {
        const prices = await (0, newstoreOrders_1.lookupPrices)(shopify, config.store, skuList);
        const total = (0, newstoreOrders_1.calculateTotal)(skuList, prices, config.ns === "sfs");
        await (0, receipts_1.generateAndAttachReceipt)({
            client: newstore,
            orderUuid,
        ...
        console.log(`  [receipt] generation failed (non-fatal): ${error.message}`);
    }
}
async function runOrderCli(argv) {
