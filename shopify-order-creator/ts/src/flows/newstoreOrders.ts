...
export type NewStoreOrderType = "SFS" | "OTC";
...
const STORE_IDS: Record<Store, string> = { US: "BRANCH_407", PS: "BRANCH_640" };
...
const NS_CUSTOMERS: Record<Store, NsCustomer> = {
...
const NS_CUSTOMER_ADDRESS: NsAddress = {
  firstName: "JJQA",
  lastName: "AutoNS",
 ...
 * NewStore Manager.
 */
export function generateExternalId(prefix: NewStoreOrderType): string {
  const suffix = randomUUID().replace(/-/g, "").slice(0, 10);
  return `QA${prefix}_${Date.now()}_${suffix}`;
}

function placedAt(): string {
  return `${new Date().toISOString().slice(0, 19)}Z`;
}

function buildItems(skus: string[], prices: Record<string, number>): Array<Record<string, unknown>> {
...
function buildCashPayment(totalAmount: number, timestamp: string): Record<string, unknown> {
  return {
    processor: "cash",
    correlation_ref: `CASH_${randomUUID().replace(/-/g, "").slice(0, 12)}`,
    type: "captured",
 ...
 */
export function buildSfsPayload(params: BuildPayloadParams): Record<string, unknown> {
 ...
 */
export function buildOtcPayload(params: BuildPayloadParams): Record<string, unknown> {
  const { store, skus, prices, externalId, placedAt: timestamp } = params;
...
export interface InjectionClients {
  newstore?: NewStoreClient;
  shopify?: ShopifyClient;
}

/** Injects a Ship From Store order into NewStore staging. */
export async function injectSfsOrder(
  store: Store,
  skus: string[],
  clients: InjectionClients = {},
): Promise<NewStoreOrderResult> {
  if (skus.length === 0) {
    throw new Error("injectSfsOrder requires at least one SKU");
  }
  const shopify = clients.shopify ?? new ShopifyClient(store);
  const newstore = clients.newstore ?? new NewStoreClient();
  ...
  const response = (await newstore.post("/v0/d/fulfill_order", payload)) as Record<string, unknown>;
  return { externalId, response };
}

/** Injects an Over the Counter order into NewStore staging. */
export async function injectOtcOrder(
  store: Store,
  skus: string[],
  clients: InjectionClients = {},
): Promise<NewStoreOrderResult> {
