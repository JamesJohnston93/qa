...
import { defaultConfig, CHERMSIDE_US, PS_STORE, type Store } from "./config";
import { DynamoClient } from "./clients/dynamo";
import { DynamoReader, orderPkFromRows, orderSkuQuantitiesFromRows, groupItemsByShipment } from "./readers/dynamoReader";
...
import { totalOrderUnits, itemCountsSettled } from "./flows/fulfilFlow";
...
const DESIGNATED_STORE: Record<Store, string> = {
  US: CHERMSIDE_US,
  PS: PS_STORE,
};

async function dumpTransactionRows(dynamo: DynamoClient, table: string, pk: string): Promise<void> {
...
function parseStore(argv: string[]): Store {
...
async function main(): Promise<void> {
  ...
  const reader = new DynamoReader(dynamo, config);
  ...
  try {
    ...
    const record = await placeOrder(config, { [sku]: SEED_QUANTITY });
    console.log(`Order ${record.orderName} (${record.orderId})`);

    const oidTail = orderIdTail(record.orderId);
    const { value: orderRows } = await pollUntil(
      () => reader.getOrderRows(store, oidTail),
      (rows) => {
        const quantities = orderSkuQuantitiesFromRows(rows);
        return quantities[sku] === SEED_QUANTITY;
      },
      60,
      2,
      "probe_order_row_settle",
      true,
    );
    ...
    const { value: items } = await pollUntil(
    ...
    const result = await rejectShipment({ reader, rejectClient: new RejectClient(), verbose: true }, orderPk, shipmentId, itemIds);
