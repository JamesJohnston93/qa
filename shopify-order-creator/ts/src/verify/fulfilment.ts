...
import type { ShipmentItem, ShipmentSummary } from "../readers/dynamoReader";
 ...
 */
export function assertShipmentItemsFulfilled(items: ShipmentItem[], shipmentId: string, orderName: string): void {
  const onShipment = items.filter((item) => item.shipmentId === shipmentId);
  if (onShipment.length === 0) {
    throw new VerificationError(
      "shipments.items_fulfilled",
      "item(s) present",
      "not found yet",
      `order ${orderName}; shipment ${shipmentId}`,
    );
  }

  const notFulfilled: Record<string, string> = {};
 ...
 */
export function assertShipmentTrackingNumber(
  ...
  if (!summary) {
    throw new VerificationError(
      ...
      "SHIPMENT# row present",
      "not found yet",
      `order ${orderName}; shipment ${shipmentId}`,
    );
  }
  if (summary.status !== FULFILLED || !summary.trackingNumber) {
    throw new VerificationError(
      "shipments.tracking_number",
      { status: FULFILLED, trackingNumber: "<non-empty>" },
      { status: summary.status, trackingNumber: summary.trackingNumber },
      `order ${orderName}; shipment ${shipmentId}`,
    );
  }
}
 ...
 */
export function assertOrderItemsFulfilled(
  orderRows: Record<string, unknown>[],
  shipmentItems: ShipmentItem[],
  orderName: string,
): void {
  const skus = new Set(shipmentItems.map((item) => item.sku));
  const relevant = orderRows.filter(
    (row) => String(row.SK ?? "").startsWith("ITEM#") && skus.has(String(row.sku ?? "")),
  );
  if (relevant.length === 0) {
    throw new VerificationError(
      "orders_table.fulfilled",
      "item row(s) present",
      "not found yet",
      `order ${orderName}`,
    );
  ...
  if (Object.keys(notFulfilled).length > 0) {
    throw new VerificationError("orders_table.fulfilled", FULFILLED, notFulfilled, `order ${orderName}`);
  }
}
