...
export const US_STORE_LOCATIONS: Record<string, string> = {
...
export const PS_STORE_LOCATIONS: Record<string, string> = {
...
export function storeLocationsFor(store: Store): Record<string, string> {
  return store === "US" ? US_STORE_LOCATIONS : PS_STORE_LOCATIONS;
}
 ...
 * it instead.
 */
export function shopifyLocationForStoreNumber(store: Store, storeNumber: string): string {
  const gid = storeLocationsFor(store)[storeNumber];
  if (!gid) {
    throw new Error(
      `No known Shopify location GID for ${store} store number "${storeNumber}" — ` +
        `extend locations.ts's mapping before asserting allocation for this store (known: ${JSON.stringify(
          Object.keys(storeLocationsFor(store)),
        )}).`,
    );
  }
  return gid;
