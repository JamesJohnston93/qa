...
export interface OrdersCaseDefinition {
  name: string;
  description: string;
  variant: OrdersCaseVariant;
  baseSku: string;
  /** The SKU an edit chain adds — only set for the four edit-driven variants. */
  addItemSku: string | null;
}
...
const VARIANT_BY_NAME: Record<string, OrdersCaseVariant> = {
  os_hold_fraud: "fraud",
...
const EDIT_DRIVEN = new Set<OrdersCaseVariant>(["outstanding", "multi", "release_payment", "partial_release"]);

export const ORDERS_CASE_NAMES = Object.keys(BASE_SLOTS);

export function buildOrdersCases(store: Store): Record<string, OrdersCaseDefinition> {
  ...
  const cases: Record<string, OrdersCaseDefinition> = {};
