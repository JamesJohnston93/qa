...
#                = N` against the same fixture set.
...
#
# Examples:
#   ./invoke-so-revision.sh --stage <stage> --profile dev --scenario 02-line-qty-increased
#   ./invoke-so-revision.sh --stage <stage> --profile dev --scenario 08-ineligible-declined
#   ./invoke-so-revision.sh --stage <stage> --profile dev --scenario 01-baseline --dry-run
#   ./invoke-so-revision.sh --stage <stage> --profile dev \
...
# │ can reach Manhattan if the senders are wired. kian-dev and staging only — any            │
# │ other stage needs --i-know-what-im-doing, and there is no good reason to pass it.        │
# └──────────────────────────────────────────────────────────────────────────────────────────┘
set -euo pipefail
...
if [[ -z "$STAGE" || -z "$PROFILE" || -z "$SCENARIO" ]]; then
	...
	echo "Example: $0 --stage staging --profile staging --scenario 02-line-qty-increased" >&2
	echo >&2
...
if [[ "$STAGE" != "kian-dev" && "$STAGE" != "staging" && "$OVERRIDE" != "true" && "$DRY_RUN" != "true" ]]; then
	echo "Refusing to publish to '${STAGE}'. This script writes; it is for kian-dev and staging." >&2
...
for line in order['lineItems']:
    ...
    # A line with no sizes carries only its style code, and the item master holds product
    # options — so it is skipped and counted, exactly as the mappers do.
    if not sizes:
        skipped_no_sizes += 1
        continue
    units = [(s['code'], s['qty']) for s in sizes]
...
idempotency_id = f'{event}#{STORE}#{ORIGIN_SYSTEM}#{reference}#{order["modifiedDate"]}#{payload_hash}'

if event == CANCEL_EVENT:
    # Mirrors the deployed buildCancelOutboundOrderCommand exactly (confirmed by direct code
    # read, BUSY-1161 slice 02): a cancel carries no items and no ship-to/address/scheduled-date/
    # customerOrderNo/echo-guard-hash, and the command carries no customerEmail key at all. Fixed
    # here 2026-09-10 after slice 02 found the CANCEL branch reused the CREATE/UPDATE shape
    # wholesale — see TOOL-NOTES.md.
    order_info = {
        'store': STORE,
    ...
    order_info = {
        ...
        # Fixed 2026-09-10 (BUSY-1161 slice 02): was 'orderDate', which the deployed builder
        # never sends and the handler's saveUnknown:false schema silently drops on save. The
        # deployed field is 'orderedAt' -- see TOOL-NOTES.md.
        'orderedAt': order['createdDate'],
        'sourceStatus': order['status'],
        'sourceStage': stage,
        'lastModified': order['modifiedDate'],
    ...
    command = {
        'orderId': order_id,
        # Full origin, matching the outbound mapper: the origin system is what keeps a sales order
        # and a branch transfer with equal references apart. Native ECOM below stays two-segment,
        # as production does — its event prefix already separates it from every other source.
        'idempotencyId': idempotency_id,
        'category': 'OUTBOUND',
        'event': event,
        'origin': f'{STORE}#{ORIGIN_SYSTEM}#{reference}',
