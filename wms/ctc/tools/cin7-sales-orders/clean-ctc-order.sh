...
# Both families live in the order's own partition, so SHIPMENT_ROWS above already carries the
# outbound header, its OUTBOUND_ITEM# lines and its TRANSACTION# rows alongside the native ones.
...
echo
echo "${SHIPMENTS_TABLE} (native and outbound share the order partition):"
...
echo "  ../common/cin7-watermark.sh --stage ${STAGE} --profile ${PROFILE} --poller so --set <ISO8601> --confirm"
