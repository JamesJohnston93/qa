...
if [[ -z "$STAGE" || -z "$PROFILE" || -z "$REFERENCE" ]]; then
	...
	echo "Example: $0 --stage staging --profile staging --reference 100234" >&2
	exit 1
fi
...
if not order_rows:
    ...
    if o.get('carrier') == 'UNASSIGNED':
        print('     ^ expected: Cin7 assigns the carrier at dispatch')
...
# Both families share the order's own partition, so the rows below and the outbound rows further
# down come from one query. Only the native family is reported here.
if not is_outbound_order:
    print(f'\nSHIPMENTS  ({len(shipments)} rows)')
    # Matched on the SK prefix, never "everything that is not an ITEM": the partition also holds
    # TRANSACTION# rows, and taking the first of those as the header reports every field as
    # absent — which reads as a broken header rather than one that has not landed yet.
    headers = [s for s in shipments if str(s.get('SK', '')).startswith('SHIPMENT#')]
    sitems  = [s for s in shipments if str(s.get('SK', '')).startswith('ITEM#')]
    stxns   = [s for s in shipments if str(s.get('SK', '')).startswith('TRANSACTION#')]

    if not headers:
        if stxns or sitems:
            print(f'  !! no SHIPMENT# header yet, but {len(stxns)} TRANSACTION# and '
                  f'{len(sitems)} ITEM# rows are present — the chain is mid-flight, or')
            print('     the CTC branch in create-shipment-items did not fire.')
            print('     Re-run in a few seconds before treating this as a fault.')
        else:
            print('  !! no shipment header. If items exist, the CTC branch in create-shipment-items')
            print('     did not fire — check the transaction origin prefix.')
    else:
        h = headers[0]
...
# The key no longer marks the family, so the header's orderType does — the same rule familyOf()
# applies on the service side.
outbound = shipments if is_outbound_order else []
...
if outbound or is_outbound_order:
    ...
    if not outbound_header_rows:
        print('  !! no outbound shipment header in the order partition. If the order exists,')
        print('     the bridge or the materialiser has not run — check check-ctc-status.sh.')
    else:
