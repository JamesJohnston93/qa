...
def scenario_single_size_line(order):
    """
    An empty `sizes[]`. The line carries only its style code, which the warehouse's item master
    does not hold — it is keyed on product options — so the line is skipped and counted rather
    than sent. Synthetic in shape: every captured order expanded into sizes, and a real empty
    `sizes[]` is a gift card, a non-inventory placeholder or a carton code.

    This is the regression test for that skip. Before it, the style code was sent as an item, the
    warehouse rejected the whole document, and the row left behind was re-sent on every later
    revision — so the order never recovered.
    """
    line = first_line(order)
    line['sizes'] = []
    line['qty'] = 3.0
