...
if [[ -z "$STAGE" || -z "$PROFILE" || -z "$REFERENCE" ]]; then
	...
	echo "Example (outbound): $0 --stage kian-dev --profile dev --reference 982409Aug26 --family outbound" >&2
	exit 1
fi
...
if [[ "$FAMILY" == "outbound" ]]; then
	...
	# The header's own PK is deterministic from company + reference, so unlike the native branch
	# below this needs no indirection through the orders table at all.
