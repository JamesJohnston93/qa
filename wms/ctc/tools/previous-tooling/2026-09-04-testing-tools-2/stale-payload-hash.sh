...
#
# ┌──────────────────────────────────────────────────────────────────────────────────────────┐
# │ WRITES one attribute on one order row. kian-dev only.                                    │
# │                                                                                          │
...
if [[ -z "$STAGE" || -z "$PROFILE" || -z "$REFERENCE" ]]; then
	...
	echo "Example: $0 --stage kian-dev --profile dev --reference 261068" >&2
	exit 1
fi

if [[ "$STAGE" != "kian-dev" && "$MODE" != "show" ]]; then
	echo "Refusing to write to '${STAGE}'. This script mutates an order row; it is for kian-dev." >&2
	exit 1
fi
...
case "$MODE" in
	...
	stale)
		...
		echo
		echo "  ./cin7-watermark.sh --poller so --stage ${STAGE} --profile ${PROFILE} \\"
