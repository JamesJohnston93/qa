...
# — and, for a WHOLESALE/RTV order, the OUTBOUND_SHIPMENT header, its OUTBOUND_ITEM lines and its
# own TRANSACTION rows, keyed off company + reference rather than the order's partition key.
#
...
# The outbound header's own PK is deterministic from company + reference — a WHOLESALE/RTV order
# never shares its shipment rows' partition key with the order's own, unlike the native lookup
# above, so this needs no indirection through ORDER_PK at all.
OUTBOUND_PK="OUTBOUND_SHIPMENT#CTC#${REFERENCE}"
...
if [[ -z "$ORDER_ROWS" && -z "$SHIPMENT_ROWS" && -z "$OUTBOUND_ROWS" ]]; then
...
echo
echo "${SHIPMENTS_TABLE} (native):"
[[ -n "$SHIPMENT_ROWS" ]] && echo "$SHIPMENT_ROWS" | sed 's/^/  /' || echo "  (none)"
echo
echo "${SHIPMENTS_TABLE} (outbound, ${OUTBOUND_PK}):"
[[ -n "$OUTBOUND_ROWS" ]] && echo "$OUTBOUND_ROWS" | sed 's/^/  /' || echo "  (none)"
echo
...
echo "${SHIPMENTS_TABLE} (outbound):"
delete_rows "$SHIPMENTS_TABLE" "$OUTBOUND_ROWS"
