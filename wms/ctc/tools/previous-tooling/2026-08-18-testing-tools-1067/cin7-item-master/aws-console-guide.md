...
**Pending (BUSY-1117, not yet in QA scope):** once BUSY-1117 deploys, the dashboard and SNS topic
below move to a separate `CatalogManhattanObservabilityStack`, shared with the CTC/Cin7 poller's
own alarms — see section 9 (also not yet in scope; QA is testing through BUSY-1116 today). Same
dashboard, renamed resource once it lands: `<stage>-catalog-manhattan-dashboard` becomes
`<stage>-catalog-manhattan-observability-dashboard`. `check-status.sh`'s CLI equivalent
auto-detects which name is actually live on a given stage, so use its output if you're unsure
which applies right now.

### Viewing the dashboard

1. Go to the **CloudWatch** console → **Dashboards** (left sidebar), region **ap-southeast-2**.
2. Open `<stage>-catalog-manhattan-dashboard` (or `<stage>-catalog-manhattan-observability-dashboard`
   once BUSY-1117 has deployed to this stage).
3. Throughput, queue depth, and DLQ-depth widgets are duplicated once per store — look for `[us]`/
   ...
   regardless of which store's item triggered it. Post-BUSY-1117, a "CTC / Cin7 poller" section
   (section 9) also sits on this dashboard.

### Checking alarm states

1. Go to the **CloudWatch** console → **Alarms** → **All alarms**, region **ap-southeast-2**.
2. Search for `<stage>-catalog-manhattan` to find the 5 UNI/PS alarms:
   ...
   (post-BUSY-1117: search `<stage>-catalog-cin7` for the 2 CTC poller alarms — section 9.)
...
## 9. Testing CTC poller resilience & alerting (Task 5B / BUSY-1117)

**Not yet in QA's current scope** — see the same note at the top of the README's Task 5B section.
QA is testing through BUSY-1116 today; this section is here for once BUSY-1117 is handed over.

Full test procedure and reasoning in the README's "Task 5B" section — this is the click-through
version of the same two alarms.

### Checking alarm states

1. Go to the **CloudWatch** console → **Alarms** → **All alarms**, region **ap-southeast-2**.
2. Search `<stage>-catalog-cin7` to find both:
   - `<stage>-catalog-cin7-cin7-poller-errors`
   - `<stage>-catalog-cin7-cin7-watermark-stale`
3. Both should show **OK** in a healthy stage (or not exist at all, on a branch predating
   BUSY-1117).

### Forcing them for real (unlike Task 5's alarms, this pair genuinely is QA-triggerable)

1. Open **Secrets Manager** → `<stage>/catalog/cin7` → note the current `username`/`apiKey`
   values somewhere safe, then **Retrieve secret value** → **Edit** → set both to garbage → save.
2. Open the **`<stage>-catalog-cin7-cin7-item-poller`** Lambda's log group (or use
   `tail-logs.sh --lambda cin7-poller`) — confirm cycles are actually failing.
3. **~9-15 minutes in:** `cin7-poller-errors` flips to `ALARM`.
4. **Left broken for a full ~2 hours:** `cin7-watermark-stale` also flips to `ALARM` — this is the
   genuine long-running soak test for `BUSY-1117.md`'s AC1/AC2, not a shortcut.
5. Restore the real secret value, confirm a cycle completes cleanly in the logs, then confirm both
   alarms return to `OK`.

**Never do this against a stage anyone else is actively testing on** — unlike the per-store UNI/PS
disruptions in section 8, breaking the Cin7 secret takes down the *entire* CTC pipeline (there's no
per-store dimension to isolate).

### Viewing the CTC dashboard section

The "CTC / Cin7 poller" section lives on the same shared dashboard as section 8 — scroll to it
after opening `<stage>-catalog-manhattan-observability-dashboard`. Widgets: products fetched per
cycle (primary vs. trigger-only), records emitted vs. skipped, option rows fetched, watermark age,
page-cap-hit, inactive skips, 429s/API calls (budget visibility only, no alarm — don't try to force
these by hitting Cin7's real rate limit), and ProductOption updates today.

### Email alerts (AC4)

Recipients live in **Systems Manager → Parameter Store** → `/catalog/manhattan-observability/alert-emails/<stage>`
(comma-separated, editable directly, no redeploy needed). Set a real test address there, then
either wait for a real alarm from the soak test above or use the same **Set alarm state** trick
from section 8 against either Cin7 alarm to confirm delivery without waiting. OQ-4 (the real
business distro address) is still open — see `BUSY-1117.md`.
