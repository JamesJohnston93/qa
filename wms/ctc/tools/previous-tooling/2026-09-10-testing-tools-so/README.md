...
Manhattan SCALE. They live outside the codebase on purpose — QA tooling, not shipped code.
...
> whole second half of this README — belongs to the item-master flow. The two traps worth naming:
>
...
**Wider Cin7 API access exists if a case ever needs it, 2026-09-01.** A Postman collection covering
Branches, Contacts, ProductOptions, ProductCategories, Stock, BranchTransfers, Cartons, Payments,
Credit Notes and Purchase Orders is held outside this repo. Two things about it: its example requests
carry no CTC branch (51908/51909 appear nowhere, while Universal Store branches and a `NEWSTORE`
credit note do), so it is probably a different account and would need a `GET /Branches` check before
being relied on for CTC; and it is not read-only, it carries create, update and delete requests
against production Sales Orders, Payments, Credit Notes and Branch Transfers.

**Do not plan around it and do not bring its credentials into this repo or into an IDE session.** JJ
runs those calls himself and writes any script that needs those credentials himself. If a test case
turns out to need an endpoint we have not scripted, say which endpoint and why, and leave the call to
him. The three CTC scripts here already cover what the sales order work has needed so far.
...
| `check-ctc-status.sh` | Queue depths and DLQs for all five stages, the poller's watermark and schedule state, and the alarms. **Start and end every test session here** | No — read-only |
...
| `inspect-ctc-order.sh` | Reads back the whole chain for one order — Order, items, address, shipment header, shipment items — and prints the CTC stamps | No — read-only |
...
| `invoke-shipment-sender.sh` | Invokes the Manhattan sender with a `SHIPMENT_CREATED` event rebuilt from a real shipment row, then prints the redacted XML and outcome lines | Only with `--send` — see the warning below |
| `clean-ctc-order.sh` | **DESTRUCTIVE.** Deletes one order and everything materialised from it, so the create path can be re-tested. `kian-dev` only — every other stage hard-fails | **Yes** — deletes rows |
...
**Re-running the create path.** The poller creates on **first sight only**, so a second run over
the same order does nothing — that is the idempotency guarantee, and it also means re-testing
needs the previous run's records removed first (`clean-ctc-order.sh`, `kian-dev` only), then the
watermark reset.
...
* **A rejected send can't be forced.** Verifying that HTTP 200 with `rejectedTransactions > 0` is
  treated as a failure needs SCALE to actually reject something. Unit-tested only.
