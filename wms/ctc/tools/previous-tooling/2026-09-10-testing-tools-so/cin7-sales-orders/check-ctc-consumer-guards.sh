...
# Added BUSY-1158 slice 02, TC4b: two of the LLD-audited consumers the original seven didn't
# cover, confirmed guarded from a live CTC reference (see results/02-consumer-stance-live-order.md).
"${STAGE}-shipping-v2-dc-packing-shipment-create|SKIP-MARKER|is a CTC record, not for uniWMS"
"${STAGE}-shipping-v2-shopify-move-fulfilment-orders|SKIP-MARKER|is a CTC record, no Shopify fulfilment order to move"
)
...
for entry in "${CONSUMERS[@]}"; do
	...
	# --output json here, not --query 'length(events)' --output text: filter-log-events can
	# require more than one underlying page in a busy shared log group, and with a text-format
	# query the CLI prints one length per page rather than one merged total, so a real match
	# sitting in the second page reads as a false 0 (found empirically 2026-08-27, see
	# TOOL-NOTES.md). --output json plus automatic pagination (no --no-paginate, no --limit)
	# merges every page's events into one array before we count it.
