...
**Tracking:** Jira project [TAA](https://universalstore.atlassian.net/jira/core/projects/TAA/board) — TS rewrite done ([TAA-13](https://universalstore.atlassian.net/browse/TAA-13)/[14](https://universalstore.atlassian.net/browse/TAA-14)/[17](https://universalstore.atlassian.net/browse/TAA-17)); PS ([TAA-22](https://universalstore.atlassian.net/browse/TAA-22)) and operator UX ([TAA-15](https://universalstore.atlassian.net/browse/TAA-15)) done; fulfilment ([TAA-21](https://universalstore.atlassian.net/browse/TAA-21), sliced [TAA-34](https://universalstore.atlassian.net/browse/TAA-34)–[TAA-39](https://universalstore.atlassian.net/browse/TAA-39)) done and live-confirmed on both stores 2026-08-23; rejection & reallocation ([TAA-31](https://universalstore.atlassian.net/browse/TAA-31)) done 2026-08-28 — both reject cases in the default suite, now 12 cases. Queued next: [TAA-32](https://universalstore.atlassian.net/browse/TAA-32) (click & collect) and [TAA-33](https://universalstore.atlassian.net/browse/TAA-33) (order-finalised), with [TAA-46](https://universalstore.atlassian.net/browse/TAA-46) and [TAA-48](https://universalstore.atlassian.net/browse/TAA-48) in flight for the wider orders-regression package. One live run outstanding under [TAA-40](https://universalstore.atlassian.net/browse/TAA-40); one deferred backend defect under [TAA-42](https://universalstore.atlassian.net/browse/TAA-42). Docs: Confluence QD space → [QA Automation Tool](https://universalstore.atlassian.net/wiki/spaces/QD/pages/1786970142/QA+Automation+Tool)

All tooling targets **staging only**.

> **Reading this cold?** `shopify-order-creator/CLAUDE.md` is the living source of truth — per-step sign-offs, live-run results, findings, and the current priority list. This README is the orientation layer. Current branch state *(2026-08-21)*: **`taa-22-ps`** is **5 commits ahead of `origin/taa-22-ps` and not yet merged to `main`** (`origin/main` is at `2783e1d`) — those commits carry the parallel-by-default flip, so nothing downstream is real until they are pushed. A second branch **`taa-34-fulfil-client`** (`829db11`) lives in a separate worktree at `../qa-taa-34` and holds fulfilment slice A. `taa-14-speedup`, `taa-15-cli-port` and `taa-17-newstore` are merged leftovers that can be pruned. *(Status update, 2026-08-28: the branch state above is stale and kept only as the record. `taa-22-ps` merged to `main` on 2026-08-21 (`3dfa0b4`) and `taa-34-fulfil-client` landed with the rest of the fulfilment slices; `main` is now at `8b18d73` with TAA-21 and TAA-31 both complete. Prunable merged leftovers are now `taa-14-speedup`, `taa-15-cli-port`, `taa-17-newstore`, `taa-22-ps`, `taa-31-reject-probe`, `taa-31-reject-transactions`, `taa-34-fulfil-client`, `taa-36-fulfil-order`, `taa-37-verify-fulfilment`, `taa-38-allocation-reflection` and `taa-39-fulfil-cases`.)*
...
Next: click & collect ([TAA-32](https://universalstore.atlassian.net/browse/TAA-32)) and the order-finalised transaction ([TAA-33](https://universalstore.atlassian.net/browse/TAA-33)), per the queue in `CLAUDE.md`. *(Status update, 2026-08-28: the fulfilment work this paragraph called "next" is done — all six slices [TAA-34](https://universalstore.atlassian.net/browse/TAA-34)–[TAA-39](https://universalstore.atlassian.net/browse/TAA-39), live-confirmed on both stores 2026-08-23 — and so is rejection & reallocation ([TAA-31](https://universalstore.atlassian.net/browse/TAA-31), phase 4): both reject cases are wired into the default suite and live-confirmed on both stores, with `TRANSACTION#` assertions added the same day in slice G.)*
